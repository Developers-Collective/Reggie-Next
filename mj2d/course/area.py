...
from levelitems import EntranceItem, SpriteItem, ZoneItem, LocationItem, ObjectItem, CommentItem
...
class Area:
    ...
    def load(self):
        ...
        self.load_tileset_names()  # block 1
        self.load_options()  # block 2
    ...
    def load_tileset_names(self):
        try:
            names = struct.unpack('>32s32s32s32s', self.blocks[0])
            self.tileset0, self.tileset1, self.tileset2, self.tileset3 = (
                name.strip(b'\0').decode('latin-1') for name in names
            )
        except Exception as e:
            raise ValueError("Failed to load tileset names") from e

    def load_options(self):
        try:
            # Unpack primary options
            data = struct.unpack('>IIHh?BxxB?Bx', self.blocks[1])
            def_events_a, def_events_b, wrap_byte, self.timeLimit, self.creditsFlag, unk_val, \
                self.startEntrance, self.ambushFlag, self.toadHouseType = data

            self.wrapFlag = bool(wrap_byte & 1)
            self.unkFlag1 = bool(wrap_byte >> 3)
            self.unkFlag2 = (unk_val == 100)
            self.defEvents = def_events_a | (def_events_b << 32)
        except Exception as e:
            raise ValueError("Error parsing general options from block 2") from e

            # Now load extended settings from block 4 if present
        self._load_extended_sprite_settings()

    def _load_extended_sprite_settings(self) -> None:
        try:
            ...
            self.unkVal1, self.unkVal2 = optdata2struct.unpack(self.blocks[3][:8])

            amount_of_settings = struct.unpack('>I', self.blocks[3][8:12])[0]
            self.spriteSettings = []

            for i in range(amount_of_settings):
                offset_index = 12 + 4 * i
                offset = struct.unpack('>I', self.blocks[3][offset_index:offset_index + 4])[0]
                setting = self._load_single_setting(self.blocks[3][offset:])
                self.spriteSettings.append(setting)
        except Exception as e:
            raise ValueError("Error loading extended sprite settings") from e

    @staticmethod
    def _load_single_setting(data: bytes) -> list:
        length = struct.unpack('>I', data[:4])[0]
        return [data[4 + 4 * i: 8 + 4 * i] for i in range(length)]
